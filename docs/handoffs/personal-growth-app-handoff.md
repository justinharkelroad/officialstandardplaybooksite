...
| **Flows — AI feedback/analysis after a session** | `analyze_flow_session`, `refine_flow_action_item`, `submit_flow_answer` path | **OpenAI** (gpt-4o-mini class) | `OPENAI_API_KEY` | Produces the session analysis + action items that feed the Playbook. |
