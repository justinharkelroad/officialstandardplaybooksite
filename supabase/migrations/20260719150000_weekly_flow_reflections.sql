-- Weekly Flow Reflections
--
-- A weekly reflection is a read-only, server-generated view over completed
-- Flow sessions. Members may select their own rows; only the service-role
-- edge function writes them. The generated document remains available while
-- a newer source set is generating or if that refresh fails.
...
CREATE TABLE public.weekly_flow_reflections (
  ...
  -- source_hash describes the latest observed source set. content_source_hash
  -- describes the source set represented by reflection_json. Keeping both is
  -- what lets a previous successful reflection stay visible during a refresh.
...
-- A member can pause Flow coach memory while a model request is already in
-- flight. Enforce that preference at the final database write boundary so the
-- request cannot commit a new synthesis after the pause becomes effective.
...
-- Weekly reflections are part of Flow coach memory. The existing member-owned
-- purge remains one atomic action across recalled insights, inline coaching,
-- and weekly cross-Flow synthesis.
